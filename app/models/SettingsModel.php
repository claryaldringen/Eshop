...
class SettingsModel extends BaseModel{
	...
	{
		return dibi::query("SELECT * FROM page")->fetchAll();
	}


	public function getPageText($pageId, $langId)
	{
		$sql = "SELECT t.* FROM [page] p
			JOIN [text] t ON t.page_id=p.id AND t.language_id=%i
			WHERE p.id=%i";

		return dibi::query($sql, $langId, $pageId)->fetch();
	}


	public function setPageText($id, $text, $pageId, $languageId)
	{
		$insert = array('id' => $id, 'page_id' => $pageId, 'language_id' => $languageId);
		if($id === NULL)dibi::query("INSERT INTO [text] %v", $insert);
		else dibi::query("UPDATE [text] SET content=%s WHERE id=%i", $text, $id);
		return $this;
	}
	...
	{
		return dibi::query("SELECT id FROM languages WHERE zkratka=%s", $short)->fetchSingle();
	}
