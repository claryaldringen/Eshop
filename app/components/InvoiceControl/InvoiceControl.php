 ...
 * @version    0.3
 */
class InvoiceControl extends NControl {
     ...
     * @param string $title
     */
    public function __construct($id = NULL, $title = NULL,$faktura = true) {
        $this->setId($id);
        $this->setTitle($title);
    ...
    public function setPodpis($title = NULL) {
        if ($title == NULL) {
            $this->podpis = '';
        } else {
            $this->podpis = $title;
    ...
    public function setPlatce($title = true) {
			$this->platce = $title;
    }

    public function setMena($title = NULL) {
        if ($title == NULL) {
            $this->mena = 'Kč';
        } else {
            $this->mena = $title;
    ...
    public function generate() {
        ...
        $template->customerAccountNumber = $this->customer->getAccountNumber();
				$template->customerAddress = $this->customer->getAddress();
        ...
        $template->podpis = $this->podpis;
        $template->platce = $this->platce;
        $template->mena = $this->mena;
        $template->faktura = $this->faktura;
    }
