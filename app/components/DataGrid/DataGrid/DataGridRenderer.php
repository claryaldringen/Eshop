 ...
 */
class DataGridRenderer extends NObject implements IDataGridRenderer
{
	...
	/** @var string */
	public $footerNFormat = '%operations% %paginator% %info%';

	/** @var string */
	public $paginatorNFormat = '%label% %input% of %count%';

	/** @var string */
	public $infoNFormat = 'Items %from% - %to% of %count% | Display: %selectbox% | %reset%';
	...
	{
		$form = $this->dataGrid->getNForm(TRUE);
		foreach ($form->getNControls() as $control) {
	...
	public function renderPaginator()
	{
		...
		$form = $this->dataGrid->getNForm(TRUE);
		$format = $this->dataGrid->translate($this->paginatorNFormat);
		$html = str_replace(
	...
	public function renderInfo()
	{
		...
		$this->infoNFormat = $this->dataGrid->translate($this->infoNFormat);
		$html = str_replace(
			array(
				'%from%',
				'%to%',
	...
	protected function generateFilterRow()
	{
		...
		$submitNControl = $form['filterSubmit']->control;
		$submitNControl->addClass($this->getValue('row.filter control .submit'));
		$submitNControl->title = $submitNControl->value;
		...
		foreach ($this->dataGrid->getColumns() as $column) {
			...
			if ($column instanceof ActionColumn) {
				...
				if ($column->hasFilter()) {
					...
					$control = $filter->getNFormNControl()->control;
		...
		if (!$this->dataGrid->hasActions()) {
			$submitNControl->addStyle('display: none');
			$row->add($submitNControl);
		}
	...
	protected function generateContentRow($data)
	{
		...
		// checker
		if ($this->dataGrid->hasOperations()) {
			$value = $form['checker'][$data[$primary]]->getNControl();
	...
	protected function generateFooterRow()
	{
		...
		$this->footerNFormat = $this->dataGrid->translate($this->footerNFormat);
		$html = str_replace(
			array(
				'%operations%',
				'%paginator%',
