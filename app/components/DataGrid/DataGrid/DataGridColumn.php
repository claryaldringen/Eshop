 ...
 */
abstract class DataGridColumn extends NComponentContainer implements IDataGridColumn
{
	...
	{
		parent::__construct();
		$this->addNComponent(new NComponentContainer, 'filters');
		$this->header = Html::el();
		$this->cell = Html::el();
	...
	{
		return $this->getNComponent('filters')->getNComponent($this->getName(), $need);
	}


	/**
	 * NFormats cell's content. Descendant can override this method to customize formating.
	 * @param  mixed
	 * @param  DibiRow|array
	 * @return string
	 */
	...
	{
		if ($this->hasFilter()) {
			$this->getNComponent('filters')->removeNComponent($this->getFilter());
		}
		$this->getNComponent('filters')->addNComponent($filter, $this->getName());
	}
}