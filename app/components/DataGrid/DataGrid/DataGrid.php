 ...
 *
 * $presenter->addNComponent($grid, 'componentName');
 * </code>
 *
 * @author     Roman Sklenář
 * @copyright  Copyright (c) 2009 Roman Sklenář (http://romansklenar.cz)
 ...
 */
class DataGrid extends NControl implements ArrayAccess, INamingContainer
{
	/** @persistent int */
	public $page = 1;
	...
	{
		$columns = new ArrayNObject();
		foreach ($this->getNComponents(FALSE, $type) as $column) {
	 ...
	 * @return void
	 */
	public function formSubmitHandler(NAppForm $form)
	{
	...
	public function render()
	{
		if (!$this->wasRendered) {
			...
			// TODO: na r20 funguje i: $this->getNForm()->isSubmitted()
			if ($this->isSignalReceiver('submit')) {
				$this->regenerateNFormNControls();
			}
		}
	...
	/**
	 * NComponent factory.
	 * @see Nette/NComponentContainer#createComponent()
	 */
	protected function createComponentNForm($name)
	{
		...
		NFormNControl::$idMask = 'frm-datagrid-' . $this->getUniqueId() . '-%s-%s';
		...
		// generate filters NFormNControls
		if ($this->hasFilters()) {
			$defaults = array();
			$sub = $form->addContainer('filters');
			foreach ($this->getFilters() as $filter) {
				$sub->addNComponent($filter->getNFormNControl(), $filter->getName());
				// NOTE: must be setted after is NFormNControl conntected to the form
	 ...
	 * @return NAppForm
	 */
	public function getNForm($need = TRUE)
	{
		return $this->getNComponent('form', $need);
	}
	 ...
	 * @return void
	 */
	protected function regenerateNFormNControls()
	{
		$form = $this->getNForm();
		...
		if ($this->hasOperations()) {
			...
			$form->removeNComponent($form['checker']);
			$sub = $form->addContainer('checker');
			foreach ($this->getRows() as $row) {
				$sub->addCheckbox($row[$this->keyName], $row[$this->keyName]);
			}
	...
	public function isSignalReceiver($signal = TRUE)
	{
		...
		//return $this->getPresenter()->isSignalReceiver($signal == 'submit' ? $this->getNForm() : $this, $signal);
	}
